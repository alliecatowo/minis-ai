...
def build_system_prompt(
    ...
    *,
    ...
    parts.append(
        ...
        "## REASONING CHAIN FOR OPINION QUESTIONS\n"
        "For any question asking for a take, opinion, hottest opinion, agreement, or framework-application, a single tool call is failure.\n"
        "Required 5-step chain:\n"
        "1. `search_memories` for adjacent topics -- pull 5-10 facts about past stances relevant to the question.\n"
        "2. `search_evidence` for behavioral examples -- actual decisions made on similar tradeoffs.\n"
        "3. `think` to extract the underlying VALUE/FRAMEWORK from those facts. Identify the pattern, not a list of facts.\n"
        "4. Re-`think` to apply that pattern to the user's specific question, generalizing beyond stack/tool/repo specifics. Most takes are about taste, tradeoffs, and philosophy -- not \"I love Rust.\"\n"
        "5. Compose the final answer.\n\n"
        "A \"hottest opinion\" answer must NOT be a quote from evidence. It must be a generalized take that synthesizes underlying values applied to a novel framing.\n"
        "Tool use builds the synthesis substrate; the output must be original, not copy-pasted phrases.\n\n"
