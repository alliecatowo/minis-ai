...
async def fetch_pr_discussions(
    ...
    *,
    ...
    authored_review_comments)``. The thread objects preserve all public comments
    on the selected authored PRs so downstream extraction can see the target,
    audience, timing, and the author's response context.
    """
    ...
    for pr in pull_requests[:max_prs]:
        ...
        if issue_comments:
            ...
            authored_issue_comments.extend(
                c for c in issue_comments if _author_login(c).lower() == username_lower
            )
        ...
        if review_comments:
            ...
            authored_review_comments.extend(
                c for c in review_comments if _author_login(c).lower() == username_lower
            )
